 ...
 */
#define VERIFY_REMAINING(n,f) if (offset+(n) > length) return FAILURE; else {info->found_offset=offset; info->found=f;}
...
parse_ipv4:
    {
        ...
        /* Check version */
        if ((px[offset]>>4) != 4)
            return FAILURE; /* not IPv4 or corrupt */
        ...
        default:
                ...
                return FAILURE; /* todo: should add more protocols, like ICMP */
        }
    }
